...
/*   Updated: 2023/02/28 16:13:16 by arobu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	philo_eat(t_philo *philo)
{
	...
	philo->meals++;
	if (((t_state *)philo->param)->rules.times_must_eat != -1 && \
		philo->meals == ((t_state *)philo->param)->rules.times_must_eat)
	{
		pthread_mutex_lock(((t_state *)philo->param)->eating);
		((t_state *)philo->param)->total_meals += philo->meals;
